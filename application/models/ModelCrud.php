...
class ModelCrud extends CI_Model
{
    public function get_data() {
        $query = $this->db->select('id_pelanggan, nama')
                          ->from('pelanggan')
    ...
    {
        $sql = "SELECT COUNT(*) AS orderan FROM pelanggan";
        $result = $this->db->query($sql);
        return $result->row()->orderan;
    }

    public function getblmselesai()
    {
        $sql = "SELECT COUNT(`status`) AS `blmselesai` FROM pelanggan WHERE `status`='Belum Selesai'";
        $result = $this->db->query($sql);
        return $result->row()->blmselesai;
    }
    public function getsdhselesai()
    {
        $sql = "SELECT COUNT(`status`) AS `sdhselesai` FROM pelanggan WHERE `status`='Selesai'";
        $result = $this->db->query($sql);
        return $result->row()->sdhselesai;
    }
